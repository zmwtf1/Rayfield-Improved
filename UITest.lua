...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'R', Name = 'Rayfield Improved Keybind'},
		-- buildwarnings
...
function RayfieldLibrary:CreateWindow(Settings)
    ...
    -- remove the old single label
    LoadingFrame.Version.Text = ""

    -- "Rayfield" part (static white text)
    local rayfieldLabel = LoadingFrame.Version:Clone()
    rayfieldLabel.Text = "Rayfield "
    rayfieldLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
    rayfieldLabel.Parent = LoadingFrame
    rayfieldLabel.Position = LoadingFrame.Version.Position

    -- "Improved" part (RGB cycling text)
    local improvedLabel = LoadingFrame.Version:Clone()
    improvedLabel.Text = "Improved"
    improvedLabel.Parent = LoadingFrame
    improvedLabel.Position = rayfieldLabel.Position + UDim2.new(0, rayfieldLabel.TextBounds.X, 0, 0)

    -- Animate "Improved"
    task.spawn(function()
        while true do
            for i = 0, 255 do
                improvedLabel.TextColor3 = Color3.fromRGB(255, i, 0)
                task.wait(0.01)
            end
            for i = 255, 0, -1 do
                improvedLabel.TextColor3 = Color3.fromRGB(i, 255, 0)
                task.wait(0.01)
            end
            for i = 0, 255 do
                improvedLabel.TextColor3 = Color3.fromRGB(0, 255, i)
                task.wait(0.01)
            end
            for i = 255, 0, -1 do
                improvedLabel.TextColor3 = Color3.fromRGB(0, i, 255)
                task.wait(0.01)
            end
            for i = 0, 255 do
                improvedLabel.TextColor3 = Color3.fromRGB(i, 0, 255)
                task.wait(0.01)
            end
            for i = 255, 0, -1 do
                improvedLabel.TextColor3 = Color3.fromRGB(255, 0, i)
                task.wait(0.01)
            end
        end
    end)
		...
		task.spawn(function()
			while true do
				...
				RayfieldLibrary:Notify({
					...
					Content = "I hope your enjoying Rayfield Improved!",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if (Settings.KeySystem) then
		...
		if not Passthrough then
			...
			KeyMain.Subtitle.Text = Settings.KeySettings.Subtitle or "Rayfield Improved Key System"
...
function RayfieldLibrary:LoadConfiguration()
	...
	if CEnabled then
		...
		elseif not success and not notified then
			warn('Rayfield Improved Configurations Error | '..tostring(result))
