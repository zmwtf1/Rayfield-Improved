...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'R', Name = 'Rayfield ' .. VersionName .. " keybind"},
		-- buildwarnings
		-- rayfieldimprovedprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			...
			if useStudio then
				file = [[
{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"Rayfield ]] .. VersionName .. [[ Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Rayfield ]] .. VersionName .. [[ Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
]]
			end
	...
	if not success then 
		if writefile then
			warn('Rayfield ' .. VersionName .. ' had an issue accessing configuration saving capability.')
		end
	end
end
...
local RayfieldLibrary = {
	...
	Theme = {
		...
		AMOLEDPurple = {
			TextColor = Color3.fromRGB(131, 75, 171),
			...
			TabStroke = Color3.fromRGB(0, 0, 0),
			TabBackgroundSelected = Color3.fromRGB(210, 210, 210),
			TabTextColor = Color3.fromRGB(131, 75, 171),
			SelectedTabTextColor = Color3.fromRGB(50, 50, 50),
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Rayfield ' .. VersionName .. ' had an issue decoding the configuration file, please try delete the file and reopen Rayfield ' .. VersionName .. ".") return end
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Rayfield " .. VersionName .. " | Unable to find '" .. FlagName .. "' in the save file.")
...
function RayfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Rayfield ' .. VersionName .. ' | Not seeing your actions in notifications?')
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Rayfield ' .. VersionName .. ' Settings', 0, true)

	if TabList['Rayfield ' .. VersionName .. '  Settings'] then
		TabList['Rayfield ' .. VersionName ..  ' Settings'].LayoutOrder = 1000
	end

	if Elements['Rayfield ' .. VersionName .. '  Settings'] then
		Elements['Rayfield ' .. VersionName .. '  Settings'].LayoutOrder = 1000
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RayfieldLibrary:Notify({Title = 'Build Mismatch', Content = 'Rayfield ' .. VersionName .. ' may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield ' .. VersionName .. ' is intended for interface build ' .. InterfaceBuild .. '.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Rayfield " .. VersionName
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface"

	if Settings.LoadingTitle ~= "Rayfield " .. VersionName then
		LoadingFrame.Version.RichText = true
        LoadingFrame.Version.Text = '<font color="rgb(240,240,240)">Rayfield</font> <font color="rgb(131,75,171)">' .. VersionName .. '</font>'
	end
		...
		task.spawn(function()
			while true do
				...
				RayfieldLibrary:Notify({
					Title = "Rayfield " .. VersionName .. " Interface",
					Content = "I hope your enjoying Rayfield " .. VersionName .. "!",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		...
		if isfile and not isfile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Rayfield " .. VersionName .. " RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Rayfield " .. VersionName .. " | " .. Key .. " Error " .. tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield ' .. VersionName .. ' specific development.')
				end
			end
		end
		...
		if not Passthrough then
			...
			KeyMain.Subtitle.Text = Settings.KeySettings.Subtitle or ("Rayfield " .. VersionName .. " Key System")
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					RayfieldLibrary:Notify({Title = "Key System", Content = "The key for this script is correct.", Image = 110139098421372})
					
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(RayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						RayfieldLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 110139098421372})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Rayfield " .. VersionName .. " | " .. ButtonSettings.Name .. " Callback Error " .. tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield ' .. VersionName .. ' specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Rayfield " .. VersionName .. " | " .. InputSettings.Name .. " Callback Error " .. tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield ' .. VersionName .. ' specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Rayfield " .. VersionName .. " | " .. DropdownSettings.Name .. " Callback Error " .. tostring(Response))
							warn('Check docs.sirius.menu for help with Rayfield ' .. VersionName .. ' specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Rayfield " .. VersionName .. " | " .. KeybindSettings.Name .. " Callback Error " .. tostring(Response))
							warn('Check docs.sirius.menu for help with Rayfield ' .. VersionName .. ' specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Rayfield " .. VersionName .. " | " .. ToggleSettings.Name .. " Callback Error " .. tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield ' .. VersionName .. ' specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Rayfield " .. VersionName .. " | " .. SliderSettings.Name .. " Callback Error " .. tostring(Response))
								warn('Check docs.sirius.menu for help with Rayfield ' .. VersionName .. ' specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			RayfieldLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 123987057699387})
		else
			RayfieldLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 110139098421372})
		end
	end
	...
	if not success then warn('Rayfield ' .. VersionName .. ' had an issue creating settings.') end
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Rayfield ' .. VersionName .. ' Settings'])
		end)
	end)
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayfieldLibrary:Notify({Title = "Rayfield " .. VersionName .. " Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "Rayfield " .. VersionName .. " Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Rayfield ' .. VersionName .. ' Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "Rayfield " .. VersionName .. "Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
